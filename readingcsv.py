...
mlt.plot(xaxis, phelps, 'b-', xaxis,lochte, 'g-', xaxis, bolt, 'y-', xaxis, random_ppl, 'r-')
...
red_patch = mpatches.Patch(color='red', label='Followers of Olympians')
blue_patch = mpatches.Patch(color='blue', label='Michael Phelps')
green_patch = mpatches.Patch(color='green', label='Ryan Lochte')
yellow_patch = mpatches.Patch(color='yellow', label='Usain Bolt')
mlt.legend(handles=[red_patch, blue_patch, green_patch, yellow_patch], loc=2)
